...
def init_socketio(app):
    ...
    def handle_subscribe(data):
        ...
        role        = data.get("role") or "unknown"     # 前端需帶入
...
def _close_session(session_id: str):
    ...
    conn = db.get_connection()
    cur  = conn.cursor()
    cur.execute("UPDATE AIChatSessions SET is_open = 0 WHERE session_id=%s",
                (int(session_id),))
    conn.commit()
    conn.close()
...
def _delete_chat_session(session_id: str):
    ...
    try:
        ...
        # 刪除 DB 紀錄
        cur.execute("DELETE FROM AIChatLogs WHERE session_id=%s", (sid_int,))
        cur.execute("DELETE FROM AIChatSessions WHERE session_id=%s", (sid_int,))
        conn.commit()
