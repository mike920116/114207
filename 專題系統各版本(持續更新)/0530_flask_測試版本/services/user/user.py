...
mail_config = config_class.get_mail_config()
...
# 初始化序列化工具與藍圖
serializer = URLSafeTimedSerializer(config_class.SECRET_KEY)
...
def forgot_password():
    ...
    try:
        ...
        # 檢查是否在 2 分鐘內已經發送過重設密碼郵件
        cursor.execute("""
            SELECT Updated_at FROM User 
            WHERE User_Email = %s AND Updated_at > DATE_SUB(NOW(), INTERVAL 2 MINUTE)
        """, (email,))
        recent_request = cursor.fetchone()
        
        if recent_request:
            logger.warning(f"重複請求重設密碼，Email: {email}")
            return render_template("user/forgot_form.html", 
                                 error_message="請等待 2 分鐘後再重新發送重設密碼郵件")
        ...
        try:
            ...
            # 更新 Updated_at 時間戳記錄請求時間
            cursor.execute("UPDATE User SET Updated_at = NOW() WHERE User_Email = %s", (email,))
            conn.commit()
