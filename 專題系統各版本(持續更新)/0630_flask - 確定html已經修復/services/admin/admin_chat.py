...
@admin_chat_bp.route("/logs/<int:sid>")
@login_required
def logs(sid):
    # 檢查用戶是否為管理員
    if not is_admin():
    ...
    # 查詢會話狀態
    cur.execute("SELECT is_open FROM AIChatSessions WHERE session_id=%s", (sid,))
...
def reply():
    ...
    # 寫入管理員訊息
    cur.execute("INSERT INTO AIChatLogs (session_id, role, message) VALUES (%s, 'admin', %s)", (sid, msg))
    # 更新會話的 updated_at，但不再自動將 need_human 設為 0，也不再將 is_open 設為 0
    # 讓 Dify API 保持暫停，直到使用者端重整頁面產生新的 session_id
    cur.execute("UPDATE AIChatSessions SET updated_at = NOW() WHERE session_id = %s", (sid,))
    conn.commit()
    conn.close()
...
def update_session_status():
    ...
    data = request.get_json()
    sid = data.get("session_id")
    is_open = data.get("is_open", 0)  # 預設關閉會話
    
    conn = db.get_connection()
    cur = conn.cursor()
    
    # 更新會話狀態
    cur.execute("UPDATE AIChatSessions SET is_open = %s WHERE session_id = %s", 
                (is_open, sid))
    
    conn.commit()
    conn.close()
    
    return jsonify({"ok": True, "session_id": sid, "is_open": is_open})
