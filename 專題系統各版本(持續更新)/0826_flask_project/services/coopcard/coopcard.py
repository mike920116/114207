...
@coopcard_bp.route('/api/dev/all_users')
@login_required
def dev_get_all_users():
    """開發測試功能 - 獲取所有用戶列表"""
    try:
        logger.info(f"[開發測試] 用戶 {current_user.id} 請求獲取所有用戶列表")
        ...
        cursor.execute("""
            ...
            FROM user 
            WHERE User_Email != %s
            ORDER BY User_name ASC, User_Email ASC
            LIMIT 100
        """, (current_user.id,))
        ...
        for user in users_data:
            ...
            # 檢查與當前用戶的好友關係狀態 - 添加 COLLATE 修復字符集問題
            cursor.execute("""
                SELECT status, requester_email, receiver_email 
                FROM friend_requests 
                WHERE (requester_email COLLATE utf8mb4_unicode_ci = %s 
            ...
            if friend_relationship:
                ...
                if status == 'accepted':
                    ...
                    friend_status = 'can_add'
                    status_text = '直接成為好友'
                    button_class = 'status-can-add'
                    button_disabled = False
            else:
                friend_status = 'can_add'
                status_text = '直接成為好友'
            ...
            # 決定顯示名稱：優先user_id，其次User_name，最後email前綴
            display_name = user[5] if user[5] else (user[1] if user[1] and user[1] != user[0] else user[0].split('@')[0])
            
            users_list.append({
                'email': user_email,
                'name': display_name,
                'full_name': user[1] or '未設置姓名',
                'avatar': get_avatar_url(user[2]),
                'bio': user[3] or '',
                'user_level': user[4] or 1,
                'user_id': user[5],
        ...
        logger.info(f"[開發測試] 返回 {len(users_list)} 個用戶")
        ...
        logger.error(f"[開發測試] 獲取所有用戶失敗: {e}")
        import traceback
        logger.error(f"[開發測試] 詳細錯誤: {traceback.format_exc()}")
        return jsonify({'success': False, 'users': [], 'error': f'獲取用戶列表失敗：{str(e)}'})

@coopcard_bp.route('/api/dev/direct_add_friend', methods=['POST'])
@login_required
def dev_direct_add_friend():
    """開發測試功能 - 直接添加好友（跳過審核）"""
    try:
        data = request.get_json()
        target_email = data.get('target_email', '').strip()
        ...
        logger.info(f"[開發測試] 用戶 {current_user.id} 直接添加好友 {target_email}")
        ...
        if existing:
            ...
            if status == 'accepted':
                ...
                # 如果是pending或rejected，先刪除舊記錄 - 添加 COLLATE
                cursor.execute("""
                    DELETE FROM friend_requests 
                    WHERE (requester_email COLLATE utf8mb4_unicode_ci = %s 
                           AND receiver_email COLLATE utf8mb4_unicode_ci = %s) 
        ...
        # 直接創建已接受的好友關係
        cursor.execute("""
            INSERT INTO friend_requests (requester_email, receiver_email, status, created_at, updated_at)
        ...
        logger.info(f"[開發測試] 成功直接添加好友關係: {current_user.id} <-> {target_email}")
        
        return jsonify({
            'success': True,
            'message': f'成功與 {target_user[1] or target_email} 成為好友！'
        })
        
    except Exception as e:
        logger.error(f"[開發測試] 直接添加好友失敗: {e}")
        import traceback
        logger.error(f"[開發測試] 詳細錯誤: {traceback.format_exc()}")
        return jsonify({'success': False, 'message': '添加好友失敗，請稍後再試'})
