...
def chat_api():
    ...
    try:
        ...
        reply  = dify_response_data.get("answer", "（AI 無回應）")
        conv_id_from_dify = dify_response_data.get("conversation_id")  # 首次回傳 conversation_id
...
def call_human():
    ...
    database_cursor.execute("""
        ...
        """, (session_id, help_msg))
    database_connection.commit()
    database_connection.close()    # 暫停 Dify
    ...
    # 🔔 發送 LINE 通知給所有管理員
    try:
        message = f"📣 使用者 {current_user.id} 呼叫真人客服，請至後台查看\n\n時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        notify_admins(message)
        logging.info(f"🔔 LINE通知已發送 (會話ID: {session_id})")
    except Exception as e:
        logging.error(f"❌ LINE通知發送失敗: {e}")
