...
def load_user(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT useracc, username, password FROM user WHERE useracc = %s", (user_id,))
...
def signup():
    try:
        # 取得使用者的輸入值
        useracc = request.form.get('useracc')
        ...
        conn = db.get_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO user (useracc, username, password) VALUES (%s, %s, %s)",
                       (useracc, username, hashed_password))
        conn.commit()
        conn.close()
...
def login():
    try:
        ...
        conn = db.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT useracc, username, password FROM user WHERE useracc = %s", (useracc,))
        result = cursor.fetchone()
        conn.close()
        ...
        if bcrypt.checkpw(password.encode('utf-8'), hashed_password):
            user = User(id=useracc, username=username, password=hashed_password)
